use super::types::{PoolOperationType, PrivacyPoolOperation};
...
impl PrivacyPoolManager {
    ...
    pub async fn join_pool(
        ...
        println!(
            "🌊 Joining pool {} with {} of {}",
            pool_id, deposit_amount, token
        );
    ...
    pub async fn generate_compliance_proof(
        ...
        println!(
            "📋 Generating compliance proof for tx {} in pool {}",
    ...
    /// Get pool balance
    pub async fn get_pool_balance(&self, pool_id: &str, token: &str) -> Result<String> {
        if !self.initialized {
            return Err(CepWalletError::NotInitialized(
                "Privacy Pools not initialized".to_string(),
            ));
...
mod tests {
    ...
    async fn test_swap_without_membership() {
        ...
        let result = manager
            .swap_in_pool("test-pool", "0x1", "0x2", "1000")
            .await;
