...
async function generateTransferProof(params) {
  ...
  const recipientAddress = params.toRailgunAddress || params.recipient;
  console.log('  To RAILGUN Address:', recipientAddress);
  console.log('  Token:', params.token);
  console.log('  Amount:', params.amount);
  ...
  try {
    ...
    if (params.mnemonic && params.encryptionKey) {
      ...
      // IMPORTANT: Refresh balances to sync with blockchain
      console.log('  Refreshing wallet balances from blockchain...');
      await refreshBalances(
        networkName,
        walletInfo.id,
        (progress) => {
          if (progress && progress.status) {
            console.log(`  Balance refresh: ${progress.status}`);
          }
        }
      );
    ...
    const erc20AmountRecipients = [{
      ...
      amount: amountWei,
      recipientAddress: recipientAddress, // Use the full address
    }];
    ...
    if (operation === 'initRailgun') {
      ...
      switch (operation) {
        ...
        case PROOF_TYPES.UNSHIELD:
          ...
          break;
        case 'scanMerkletree':
          result = await scanMerkletree(operationParams);
          break;
        default:
 ...
 * 
 * @param {Object} params
 * @param {string} params.railgunWalletId - RAILGUN wallet ID to scan for
 * @param {number} params.chainId - Chain ID
 * @returns {Object} { success, scannedBlockCount }
 */
async function scanMerkletree(params) {
  console.log('[ProofGen] Scanning merkletree for wallet:', params.railgunWalletId);
  
  try {
    const { rescanFullUTXOMerkletreesAndWallets, refreshBalances } = require('@railgun-community/wallet');
    
    const networkName = chainIdToNetworkName(params.chainId || 11155111);
    
    // First, rescan full UTXO merkletrees
    console.log('[ProofGen] Rescanning full UTXO merkletrees...');
    await rescanFullUTXOMerkletreesAndWallets(
      networkName,
      (progress) => {
        if (progress && progress.status) {
          console.log(`[ProofGen] Rescan: ${progress.status}`);
        }
      }
    );
    
    console.log('[ProofGen] Rescan completed, now refreshing balances...');
    
    // Then refresh balances for the specific wallet
    await refreshBalances(
      networkName,
      params.railgunWalletId,
      (progress) => {
        if (progress && progress.status) {
          console.log(`[ProofGen] Balance refresh: ${progress.status}`);
        }
      }
    );
    
    console.log('[ProofGen] Merkletree scan completed ✓');
    ...
    console.error('[ProofGen] Merkletree scan failed:', error.message);
    return {
      success: false,
      error: error.message,
    };
